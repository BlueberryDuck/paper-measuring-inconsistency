...
\begin{example}
    Consider the logic program \(P = \{a \leftarrow \text{not } a, b., a \leftarrow \text{not }c., d \leftarrow \text{not }d., b., c., d.\}\) which has \(SI_{\min}(P) = \{\{a \leftarrow \text{not }a,b., b., c.\}\}\). The formulas \(\alpha = \{d.\}\) and \(\beta = \{a \leftarrow \text{not }c.\}\) are in \(Free_{SI}(P)\). Although they are free, removing them affects \(SI_{\min}(P)\): Removing \(\alpha\) turns \(\{d \leftarrow \text{not }d.\}\) into a strongly inconsistent subset, while removing \(\beta\) makes \(\{c.\}\) irrelevant regarding the present conflict, so \(SI_{\min}(P \backslash \{\alpha, \beta\}) = \{\{a \leftarrow \text{not }a,b., b.\}, \{d \leftarrow \text{not }d.\}\}\).
\end{example}
