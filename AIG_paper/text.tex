...
\begin{definition}[\cite{brewka_strong_2019}]
    A logic \(L\) is a tuple \(L = (WF, BS, INC, ACC)\), where \(WF\) is a set of well-formed formulas, \(BS\) is a set of belief sets, \(INC \subseteq BS\) is an upward closed set of inconsistent belief sets, and \(ACC: 2^{WF} \rightarrow 2^{BS}\) assigns a collection of belief sets to each subset of \(WF\). A knowledge base \(\mathcal{K}\) of \(L\) is finite subset of \(WF\). A knowledge base \(\mathcal{K}\) is called inconsistent if \(ACC(\mathcal{K}) \subseteq INC\).
\end{definition}
...
Logic programs can be modeled regarding a set \(A\) of propositional atoms as logic \(L_A^{ASP} = (WF_A^{ASP}, BS_A^{ASP}, INC_A^{ASP}, ACC_A^{ASP})\). \(WF_A^{ASP}\) is the set of all rules over \(A\), \(BS_A^{ASP}\) consists of the sets of literals over \(A\), \(INC_A^{ASP} = \{Lit(A)\}\), and \(ACC_A^{ASP}\) assigns the sets of all answers sets to its respective logic program.
...
\begin{example}
    ...
    \[\begin{array}{rrrr}
            ...
            P': &    & b. & \neg b \leftarrow \text{not a}.
        \end{array}\]
    Then \(ACC_A^{ASP}(P) = \{\{a,b\}\}\) and \(ACC_A^{ASP}(P') = \{\}\). Observe that \(P\) is consistent while \(P'\) is inconsistent.
\end{example}
